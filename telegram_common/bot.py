...
from .audio_utils import AudioFileManager, format_file_size, validate_audio_size
...
async def check_user_access(
    user_data: dict, update: Update, bot_config: dict = None
) -> bool:
    ...
    if speech_only:
        ...
        try:
            ...
            opening_generation_prompt = [
                ...
                {
                    "role": "user",
                    "content": "Generate a brief, friendly opening message for a new user. Ask for their name and how you can help them. Stay fully in character and be welcoming. Keep it under 60 words. You may use markdown formatting for emphasis.",
                },
            ]

            logger.info(f"Generating personalized opening for user {user_id}")
            opening_message = await model_client.generate_response(
                opening_generation_prompt
            )
            ...
            if not opening_message or opening_message.strip() == "":
                ...
                logger.warning(
                    f"AI opening generation failed for user {user_id}, using fallback"
                )
...
def markdown_to_telegram_html(text):
    ...
    pattern = re.compile(
        r"(</?(b|i|s|a|code|pre)\b[^>]*>)|([<>&])", flags=re.IGNORECASE
    )
...
async def process_user_message(
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"Bot response: {response_text} ({len(response_text)} characters)"
            )
        except Exception as e:
        ...
        logger.error(
            f"Error processing {message_type} message: {str(e)}, type: {type(e).__name__}"
        )
        await update.message.reply_text(
...
async def handle_photo_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        model_client = context.bot_data["model_client"]
        if (
            not hasattr(model_client, "supports_vision")
            or not model_client.supports_vision()
        ):
            await update.message.reply_text(
    ...
    try:
        ...
        if await processed_updates.contains.aio(update_id):
            ...
            if isinstance(value, dict) and value.get("status") == "processing":
                ...
                logger.info(
                    f"Update {update_id} is already being processed for {processing_time:.1f} seconds"
                )
...
async def clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await conversations.contains.aio(user_id):
        ...
        if speech_only:
            ...
            # Premium user
            response_msg = (
                "Conversation history has been cleared! (Premium - unlimited messages)"
            )
        else:
...
async def handle_text_to_speech(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not text.strip():
            await update.message.reply_text(
                "Please send some text to convert to speech."
            )
            return
    ...
    if speech_only:
        ...
        async def handle_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if not (
                ...
                or doc.file_name.lower().endswith(
                    (".aac", ".m4a", ".mp3", ".ogg", ".wav", ".webm")
                )
            ):
                await update.message.reply_text(
                    "Sorry, I only handle audio files for transcription."
                )
                return
